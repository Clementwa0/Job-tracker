...
export interface UpdateProfileDto { name?: string; email?: string; jobTitle?: string; location?: string; phone?: string; }
...
export const authService = {
  async register(data: RegisterRequest) {
    const r = await axiosInstance.post<AuthResponse>("/auth/register", data);
    return r.data;
  },
  async login(data: LoginRequest) {
    const r = await axiosInstance.post<AuthResponse>("/auth/login", data);
    return r.data;
  },
  async logout() {
  ...
  async updateProfile(data: UpdateProfileDto) {
    const r = await axiosInstance.patch<{ success: true; data: { user: User } }>("/auth/me", data);
    return r.data;
  },
