...
import { extractKeywords } from "./atsScore";
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Tailor to a job</h3>
        </div>
        <button
          type="button"
          onClick={runAi}
      ...
      {jd && (
        <div className="mt-2 flex flex-wrap gap-1">
          {localKeywords.slice(0, 12).map((k) => (
            <span key={k} className="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] text-gray-700 dark:bg-gray-800 dark:text-gray-300">
              {k}
            </span>
          ))}
        </div>
      ...
      {match && (
        <div className="mt-3 space-y-2 rounded-lg border border-gray-100 bg-gray-50 p-3 dark:border-gray-800 dark:bg-gray-950/50">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-700 dark:text-gray-200">Match score</span>
            <span className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{match.matchScore}</span>
          </div>
          {match.summary && <p className="text-xs text-gray-600 dark:text-gray-300">{match.summary}</p>}
          {match.keywords?.missing && match.keywords.missing.length > 0 && (
            <div>
              <p className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-rose-600">Missing keywords</p>
              <div className="flex flex-wrap gap-1">
                {match.keywords.missing.slice(0, 12).map((k) => (
                  <span key={k} className="rounded-full bg-rose-50 px-2 py-0.5 text-[10px] text-rose-700 dark:bg-rose-950/40 dark:text-rose-300">
                    {k}
                  </span>
                ))}
              </div>
