...
import { resumeService } from "@/features/jobseeker/resumes/services/resume.client";
import { migrateLegacyResumes } from "@/lib/legacyLocalData";
import {
  EMPTY_RESUME,
  normalizeResume,
  uid,
...
export function useResumesIndex() {
  ...
  const refresh = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // Import resumes an older version kept in this browser (once), then load from the API.
      if (!migrated.current) {
        migrated.current = true;
        await migrateLegacyResumes((resume) => resumeService.create(resume)).catch(() => {
          /* leave local copies in place; retried on the next visit */
        });
      }
