...
export function EducationEditor({
  ...
  if (items.length === 0) return <p className="text-sm text-muted-foreground">No education entries yet.</p>;
  return (
    <div className="space-y-3">
      {items.map((ed) => (
        <div key={ed.id} className="rounded-lg border border-border p-3">
...
export function ProjectsEditor({
  ...
  if (items.length === 0) return <p className="text-sm text-muted-foreground">No projects yet.</p>;
  return (
    <div className="space-y-3">
      {items.map((p) => (
        <div key={p.id} className="rounded-lg border border-border p-3">
          <div className="flex items-start justify-between gap-2 mb-3">
            <p className="text-xs font-semibold text-foreground">{p.name || "New project"}</p>
            <button
              type="button"
              onClick={() => remove(p.id)}
              className="rounded p-1 text-muted-foreground hover:text-red-500"
...
export function SkillsEditor({
  ...
  if (items.length === 0) return <p className="text-sm text-muted-foreground">No skill groups yet.</p>;
  return (
    <div className="space-y-3">
      {items.map((g) => (
        <div key={g.id} className="rounded-lg border border-border p-3">
          <div className="flex items-start justify-between gap-2 mb-3">
            <p className="text-xs font-semibold text-foreground">{g.category || "New group"}</p>
            <button
              type="button"
              onClick={() => remove(g.id)}
              className="rounded p-1 text-muted-foreground hover:text-red-500"
