  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        <div className="rounded-full bg-muted bg-card p-4 mb-4">
          <Briefcase className="h-8 w-8 text-muted-foreground dark:text-muted-foreground" />
        </div>
        <h3 className="text-sm font-semibold text-foreground text-muted-foreground mb-1">
          No experience added yet
        </h3>
        <p className="text-xs text-muted-foreground dark:text-muted-foreground max-w-sm">
...
function ExperienceItem({
  ...
  return (
    <div className="group relative rounded-xl border border-border bg-card p-4 shadow-sm transition-all hover:shadow-md bg-card/50 sm:p-5">
      {/* Header */}
      <div className="mb-4 flex items-start gap-3">
        <div className="hidden sm:flex items-center mt-1 text-muted-foreground/40 group-hover:text-muted-foreground transition-colors cursor-grab">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground dark:text-muted-foreground">
            {(item.startDate || item.endDate || item.current) && (
              <div className="flex items-center gap-1">
                ...
                {getDuration() && (
                  <span className="text-muted-foreground dark:text-muted-foreground ml-1">{getDuration()}</span>
                )}
              </div>
            )}
            {item.location && (
        ...
        <button
          ...
          className="rounded-lg p-2 text-muted-foreground hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20 dark:hover:text-red-400 transition-colors flex-shrink-0"
      ...
      <label className="mt-4 inline-flex items-center gap-2.5 text-sm text-foreground text-muted-foreground cursor-pointer group/toggle">
        <div className="relative">
          ...
          />
          <div className="w-9 h-5 bg-muted rounded-full peer-checked:bg-blue-600 transition-colors" />
        ...
        <span className="group-hover/toggle:text-foreground transition-colors">
...
function BulletRow({
  ...
  return (
    <li className="relative">
      <div className={`flex items-start gap-2 p-2 -mx-2 rounded-lg transition-colors ${isFocused ? 'bg-muted bg-card' : ''}`}>
        <span className="mt-3 text-muted-foreground/40 select-none flex-shrink-0">
          {index + 1}.
        </span>
        <textarea
          rows={2}
        ...
        <div className="flex items-center gap-1 mt-1 flex-shrink-0">
          ...
          <button
            type="button"
            onClick={onRemove}
            className="rounded-lg p-2 text-muted-foreground hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 dark:hover:text-red-400 transition-colors"
          ...
          <div className="p-2 space-y-1">
            {variants.map((v, i) => (
              <button
                ...
                className="group/suggestion block w-full text-left text-sm rounded-lg px-3 py-2.5 hover:bg-card hover:bg-muted text-foreground transition-all hover:shadow-sm"
              >
