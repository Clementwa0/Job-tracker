...
export function buildTrackerPayloadFromPosting(job: PublicJobDetail | PublicJobListItem): JobPayload {
  const currency = job.salaryCurrency || "KES";
  const detail = "applyMethod" in job;
  return {
    jobTitle: job.title,
...
export function useAddPostingToTracker() {
  ...
  const addToTracker = useCallback(async (job: PublicJobDetail | PublicJobListItem) => {
    try {
      setIsAdding(true);
      setError(null);
