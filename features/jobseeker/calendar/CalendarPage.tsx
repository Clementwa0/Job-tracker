...
import AgendaSidebar from "@/features/jobseeker/calendar/components/AgendaSidebar";
import CalendarLegend from "@/features/jobseeker/calendar/components/CalendarLegend";
import EventDetailsDrawer from "@/features/jobseeker/calendar/components/EventDetailsDrawer";
  ...
  return (
    <div className="space-y-3 md:space-y-4">
      ...
      <div className="rounded-2xl border border-border bg-card p-3 md:p-4 shadow-sm">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div
            ...
            className="inline-flex rounded-lg border border-border bg-muted bg-card/60 p-0.5 text-xs md:text-sm ml-auto"
          >
            ...
            <div className="flex md:hidden">
              {MOBILE_VIEWS.map((v) => (
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-md font-medium transition ${ view === v ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground" }`}
                >
                  {VIEW_LABELS[v]}
                </button>
              ))}
            ...
            <div className="hidden md:flex">
              {ALL_VIEWS.map((v) => (
                <button
                  ...
                  aria-selected={view === v}
                  onClick={() => changeView(v)}
                  className={`px-3 py-1.5 rounded-md font-medium transition ${ view === v ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground" }`}
                >
                  {VIEW_LABELS[v]}
                </button>
              ))}
        ...
        <div className="flex items-center gap-2 mt-3 md:hidden">
          ...
          <button
            onClick={() => nav("today")}
            className="flex-1 text-xs font-medium py-1.5 rounded-lg border border-border text-muted-foreground hover:bg-muted transition"
          >
            Today
          </button>
          <button
          ...
          <button
            ...
            className={`ml-1 px-3 py-1.5 rounded-lg border text-xs font-medium transition ${ sidebarOpen ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400" : "border-border text-muted-foreground hover:bg-muted" }`}
          >
            Upcoming
          </button>
        </div>
      ...
      <div className="grid gap-3 md:gap-4 lg:grid-cols-[1fr_320px]">
        <div className="rounded-2xl border border-border bg-card p-2 md:p-4 shadow-sm fc-themed">
          <FullCalendar
            ref={calendarRef}
            ...
            eventDisplay="block"
            dayHeaderClassNames="text-[10px] md:text-xs font-semibold uppercase tracking-wide text-muted-foreground bg-card dark:text-muted-foreground"
            ...
            dayCellClassNames="hover:bg-muted hover:bg-muted/40 transition-colors"
            noEventsContent={() => (
              <div className="py-10 text-center text-sm text-muted-foreground">No events in this range.</div>
            )}
          />
        </div>
...
// FullCalendar measures the DOM on mount, so this page is rendered client-only.
export default dynamicImport(() => Promise.resolve(Calendar), { ssr: false });
